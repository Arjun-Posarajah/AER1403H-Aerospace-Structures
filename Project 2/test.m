...
dN_dx = (J \ [dN_dxi; dN_deta])';
            ...
            k_element = k_element + B' * elastic_modulus * (1 - poisson_ratio) / ((1 + poisson_ratio) * (1 - 2 * poisson_ratio)) * B * det(J);
        end
    end
    
for i = 1:4
    for j = 1:4
        index_i = [2 * element_nodes(i) - 1, 2 * element_nodes(i)];
        index_j = [2 * element_nodes(j) - 1, 2 * element_nodes(j)];
        K_global(index_i, index_j) = K_global(index_i, index_j) + k_element(2 * i - 1:2 * i, 2 * j - 1:2 * j);
    end
end
end 