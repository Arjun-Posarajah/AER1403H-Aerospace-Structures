...
width = 20E-3; 
thickness = 2E-3; 
numtheta = 6;                    % Circumferential Number of Elements
numradial = 6;                   % Radial Number of Elements
...
middle_node = [];
middle_thickness_index = ceil(numradial / 2); % Middle thickness index

for i = 1:(numtheta + 1)
    node_index = (middle_thickness_index * (numtheta + 1)) + i;
    if nodes(node_index, 2) == 0 % Check if the node is on the middle thickness
        middle_node = nodes(node_index, :);
...
for i = 1:(numtheta + 1)
    node_index = (middle_thickness_index * (numtheta + 1)) + i;
    if nodes(node_index, 2) == 0 % Check if the node is on the middle thickness
        middle_node_index = node_index;
        break; % Stop the loop once the node is found
    end
end
forcedof=middle_node_index*2;
...
displacement= K\applied_force_vector(free_dof);

[row_indices, col_indices] = find(displacement);
newnodes = nodes; % Initialize newnodes with the original nodes
...
for i = 1:length(col_indices)
    ...
    if rem(c, 2) == 1  % If c is odd, update x-coordinate
        cx = (c + 1) / 2;
        newnodes(r, 1) = newnodes(r, 1) + displacement(i);
    elseif rem(c, 2) == 0  % If c is even, update y-coordinate
        cy = c / 2;
        newnodes(r, 2) = newnodes(r, 2) + displacement(i);
    end
end
